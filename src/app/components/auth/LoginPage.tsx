      ...
      <div className="w-full max-w-md mx-4">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={submitting || !email || !password} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all">
              {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ArrowRight className="w-4 h-4" />Entrar</>}
            </button>
          </form>
          ...
          <button onClick={onSwitchToSignup} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition-colors">Criar uma conta</button>
        </div>
        <p className="text-center text-xs text-gray-400 mt-6">Axon v4.2 — Plataforma Educativa para Medicina</p>
      </div>
    </div>
  );
}
