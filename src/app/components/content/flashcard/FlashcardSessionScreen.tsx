...
import { RATINGS } from '@/app/hooks/useFlashcardSession';
      ...
      <div className="flex-1 flex items-center justify-center p-0 relative z-10 min-h-0">
        <div className="relative w-full h-full bg-white shadow-2xl shadow-black/40 overflow-hidden flex flex-row">
          <div className="flex-1 flex flex-col min-h-0 min-w-0 relative">
            <motion.div layout className={clsx(
              ...
              <div className="flex justify-center mb-4 pointer-events-none">
                <SpeedometerGauge cards={cards} currentIndex={currentIndex} sessionStats={sessionStats} />
              </div>
              ...
              <motion.h3 layout="position" className={clsx(
              ...
              {isRevealed && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
                  ...
                  <div className="w-full max-w-xl mx-auto grid grid-cols-5 gap-2">
                    {RATINGS.map((rate) => (
                      <button key={rate.value} onClick={() => handleRate(rate.value)}
          ...
          <div className="hidden lg:flex w-[38%] shrink-0 border-l border-gray-100 bg-gray-50/30 flex-col relative overflow-hidden">
            ...
            <AnimatePresence mode="wait">
              <motion.div key={`${isRevealed ? 'revealed' : 'question'}-${currentIndex}`}
                ...
                {currentCard.image ? (
                  <img src={currentCard.image} alt={currentCard.question}
                    ...
                    style={currentCard.imagePosition ? {
                      objectPosition: `${currentCard.imagePosition.x}% ${currentCard.imagePosition.y}%`,
                      transform: `scale(${currentCard.imagePosition.scale})`,
                    } : undefined}
                  />
                ) : (
                          ...
                          <div key={i} className="flex-1 h-1.5 rounded-full transition-all duration-300"
                            style={{
                              backgroundColor: i < currentIndex
                                ? sessionStats[i] >= 4 ? '#22c55e' : sessionStats[i] >= 3 ? '#eab308' : sessionStats[i] >= 1 ? '#ef4444' : '#d1d5db'
                                : i === currentIndex ? '#14b8a6' : '#e5e7eb',
