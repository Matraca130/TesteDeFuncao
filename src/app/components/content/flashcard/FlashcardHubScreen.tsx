...
import { groupSectionsByRegion } from '@/app/data/courses';
...
export function HubScreen({ sections, allCards, courseColor, courseName, onOpenSection, onStartAll, onBack }: {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -30 }} className="h-full overflow-y-auto bg-surface-dashboard">
      <div className="relative px-8 pt-4 pb-6 bg-white overflow-hidden border-b border-gray-200/80">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-end justify-between gap-8 mb-4">
            ...
            {allCards.length > 0 && (
              <button onClick={onStartAll} className="flex items-center gap-2 px-5 py-2.5 bg-teal-600 hover:bg-teal-700 rounded-lg text-white font-medium text-sm transition-colors shrink-0">
          ...
          <div className="flex items-center justify-between">
            ...
            {allCards.length > 0 && (
              <div className="hidden md:flex items-center gap-5">
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-500" /><span className="text-xs text-gray-500"><span className="text-emerald-600 font-semibold">{stats.mastered}</span> dominados</span></div>
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-amber-500" /><span className="text-xs text-gray-500"><span className="text-amber-600 font-semibold">{stats.learning}</span> aprendendo</span></div>
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-rose-500" /><span className="text-xs text-gray-500"><span className="text-rose-600 font-semibold">{stats.newCards}</span> a revisar</span></div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="px-6 py-6 bg-surface-dashboard">
        <div className="max-w-5xl mx-auto space-y-8">
          {(() => {
            const regionGroups = groupSectionsByRegion(sections);
              ...
              <div key={group.region}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 auto-rows-min">
                  {group.sections.map((section, idx) => {
                    ...
                    return (
                      <motion.button key={section.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.06 }}
                        whileHover={{ y: -2 }} whileTap={{ scale: 0.99 }} onClick={() => onOpenSection(section, sections.indexOf(section))}
                        className={clsx("bg-white rounded-xl p-5 text-left border border-gray-200 group relative overflow-hidden transition-all duration-200", "hover:border-gray-300 hover:shadow-md")}>
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                <h3 className="font-semibold text-gray-900 truncate" style={headingStyle}>{section.title}</h3>
                                <span className="text-sm font-semibold text-teal-600 shrink-0 ml-2">{Math.round(sStats.pct)}%</span>
                              </div>
                              <p className="text-xs text-gray-400 uppercase tracking-wide">{section.topics.length} topicos</p>
                            </div>
                          </div>
