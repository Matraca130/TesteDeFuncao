// ══════════════════════════════════════════════════════════════
// AXON — Flashcard Deck Screen (pure frontend)
// ══════════════════════════════════════════════════════════════
        ...
        <div className="px-8 pt-6 pb-5">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              {cards.length > 0 && (
                <button onClick={() => onStart(filteredCards.length > 0 ? filteredCards : cards)}
                  className="flex items-center gap-3 px-8 py-4 rounded-2xl text-white text-lg font-bold shadow-sm hover:scale-105 active:scale-95 transition-all bg-teal-600 hover:bg-teal-700">
            ...
            <div className="flex items-center gap-5 flex-1">
              ...
              <div className="flex items-center gap-1.5">
                {[
                  ...
                  <button key={f.key} onClick={() => setFilterMastery(f.key)}
                    className={clsx("px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all",
                      filterMastery === f.key ? `${f.color} ring-1 ring-current/20 shadow-sm` : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"
                    )}>
                    {f.label} ({f.count})
                  </button>
                ))}
        ...
        <div className="h-full">
          {filteredCards.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              ...
              <p className="text-sm font-medium">{cards.length === 0 ? 'Nenhum flashcard neste deck' : 'Nenhum card nesta categoria'}</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 auto-rows-fr h-full">
              {filteredCards.map((card, idx) => (
                <motion.div key={card.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}
                  ...
                  <div className="flex-1 flex flex-col p-1.5 min-h-0 overflow-hidden gap-0.5">
                    <div className="flex items-center justify-between shrink-0">
                      <div className="w-3.5 h-3.5 rounded flex items-center justify-center text-[6px] font-bold bg-teal-50 text-teal-600">{idx + 1}</div>
                      <div className="flex items-center gap-px">
                        {[1, 2, 3, 4, 5].map(level => (
                          <div key={level} className={clsx("w-0.5 h-1 rounded-full transition-colors", level <= card.mastery ? "bg-teal-500" : "bg-gray-200")} />
                        ))}
                      </div>
                    </div>
