// ============================================================
// MarkdownRenderer — Simplified markdown to React elements
// ============================================================
      ...
      return codeParts.map((cp, ci) => {
        if (cp.startsWith('`') && cp.endsWith('`')) {
          return (
            <code
              key={`${i}-${ci}`}
              className="bg-gray-100 text-violet-600 px-1 py-0.5 rounded text-xs font-mono"
            >
              {cp.slice(1, -1)}
            </code>
          );
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed.startsWith('### ')) {
      ...
      elements.push(
        <blockquote
          key={i}
          className="border-l-3 border-violet-400 pl-3 my-2 text-violet-700 bg-violet-50 py-2 pr-3 rounded-r-lg text-xs italic"
        >
