...
import { colorBgMap, colorAccentMap } from './types';
          ...
          >
            <div className="bg-white border-x border-b border-gray-200 rounded-b-xl shadow-lg max-h-80 overflow-y-auto custom-scrollbar">
              {[...annotations].reverse().map((ann) => (
                <div key={ann.id} className={clsx("p-3 border-b border-gray-100 last:border-0 relative group", colorBgMap[ann.color])}>
                  {/* Color indicator bar */}
                  <div className={clsx("absolute left-0 top-0 bottom-0 w-1 rounded-l", colorAccentMap[ann.color])} />
