...
import type { Semester, Section } from '@/app/data/courses';
  ...
  return (
    <div className="h-full bg-slate-50/50 overflow-hidden">
      <AnimatePresence mode="wait">
        {viewState === 'selection' ? (
          <QuizSelection key="selection" onSelect={handleTopicSelect} />
        ) : (
...
function QuizSelection({ onSelect }: { onSelect: (topic: Topic) => void }) {
                    ...
                    <div key={group.region}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {group.sections.map((section: Section) => {
                          ...
                          return (
                          <div key={section.id} className="bg-white rounded-2xl border border-gray-100 hover:border-teal-200 transition-all shadow-[0_2px_8px_rgba(0,0,0,0.02)] hover:shadow-lg flex flex-col h-fit">
                            <button
                              type="button"
                                  ...
                                  <div key={subGroup.subcategory}>
                                    ...
                                    <div className="space-y-0.5">
                                      {subGroup.topics.map((topic: Topic) => {
                                        ...
                                        // ── Topic WITH subtopics → expandable folder ──
                                        if (hasSubtopics) {
                                          const totalSubQuizzes = topic.subtopics!.reduce((sum, st) => sum + (st.quizzes?.length || 0), 0);
                                          const subWithQuiz = topic.subtopics!.filter(st => st.quizzes && st.quizzes.length > 0).length;
                                          return (
                                            <div key={topic.id}>
                                              <button
                                        ...
                                        // ── Topic WITHOUT subtopics → direct button (current behavior) ──
                                        return (
              ...
              {qType === 'multiple-choice' && currentQ.options && (
                <div className="space-y-3 mb-6">
                  {currentQ.options.map((option, idx) => {
                    ...
                    return (
                      <button key={idx} onClick={() => !isReviewing && setLiveSelectedOption(idx)} disabled={isReviewing}
              ...
              {qType === 'write-in' && (
                <div className="mb-6">
                  <div className={clsx("rounded-xl border-2 overflow-hidden transition-all",
                    ...
                    <textarea value={textAnswer} onChange={(e) => setLiveTextInput(e.target.value)} disabled={isReviewing}
                        ...
                        <React.Fragment key={i}>
                          ...
                          {i < arr.length - 1 && (
                            <span className="inline-block align-bottom mx-1">
                              <input type="text" value={textAnswer} onChange={(e) => setLiveTextInput(e.target.value)} disabled={isReviewing}
