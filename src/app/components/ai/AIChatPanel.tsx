      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-purple-50">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 truncate">
            {keywordTerm ? `Contexto: ${keywordTerm}` : 'Tutor de Medicina'}
          </p>
        </div>
        {messages.length > 0 && (
          <button
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-8">
            ...
            <p className="text-sm text-gray-500">
              {keywordTerm
                ? `Pergunte sobre "${keywordTerm}"`
                : 'Faca uma pergunta sobre medicina'}
            </p>
      ...
      <div className="border-t border-gray-100 p-3">
        <div className="flex items-end gap-2">
          <textarea
            ...
            placeholder={keywordTerm ? `Pergunte sobre ${keywordTerm}...` : 'Digite sua pergunta...'}
            disabled={sending || !isAuthenticated}
            rows={1}
            className="flex-1 resize-none px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent disabled:opacity-50 transition"
            style={{ maxHeight: '120px' }}
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || sending || !isAuthenticated}
            className="p-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-md transition-all"
          >
            {sending ? (
