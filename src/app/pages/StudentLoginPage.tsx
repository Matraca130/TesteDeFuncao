// ============================================================
// Axon v4.4 — Student Login Page (/i/:slug/login)
// ============================================================
    ...
    <div className="min-h-screen bg-[#f5f2ea] flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center gap-3 mb-8">
          ...
          <div className="text-center"><h1 className="text-2xl font-bold text-gray-900">{institution?.name || 'Entrar'}</h1><p className="text-sm text-gray-500 mt-1">Acesso para alunos</p></div>
        </div>
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1.5">Senha</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500" disabled={isLoading} autoComplete="current-password" /></div>
          {error && <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-600">{error}</div>}
          <button type="submit" disabled={isLoading} className="w-full py-2.5 bg-amber-600 text-white rounded-xl font-semibold text-sm hover:bg-amber-700 disabled:opacity-50 flex items-center justify-center gap-2">{isLoading ? <><Loader2 size={16} className="animate-spin" /> Entrando...</> : 'Entrar'}</button>
        </form>
        <p className="text-center text-xs text-gray-500 mt-4">Nao tem conta? <button onClick={() => navigate(`/i/${slug}/signup`)} className="text-amber-600 font-semibold hover:underline">Criar conta</button></p>
      </div>
    </div>
  );
}

export function StudentLoginPage() { return <RequireGuest><StudentLoginForm /></RequireGuest>; }
