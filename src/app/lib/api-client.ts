...
export async function updateVideo(idOrSummaryId: string, dataOrId: any, maybeData?: any): Promise<any> {
  let id: string; let data: any;
  if (typeof dataOrId === 'string' && maybeData) {
    id = dataOrId; data = maybeData;
  ...
  } else { id = idOrSummaryId; data = dataOrId; }
  if (USE_MOCKS) {
    await delay();
...
export async function createVideoNote(
  videoId: string, noteTextOrStudentId: string,
  timestampSecondsOrNoteText: number | string | null, maybeTimestampMs?: number | null,
): Promise<VideoNote> {
  let noteText: string; let timestampSeconds: number | null;
...
export async function createStudyPlan(data: any): Promise<any> {
  if (USE_MOCKS) {
    ...
    const plan: StudyPlan = {
      id: mockId('sp'), title: data.title || data.name || 'Novo Plano',
      description: data.description || '',
      start_date: data.start_date || now().split('T')[0],
      end_date: data.end_date || data.target_date || '',
      progress: 0, items: data.items || [], created_at: now().split('T')[0],
    };
...
export async function fetchContentHierarchy(): Promise<{
  ...
  if (USE_MOCKS) {
    ...
    store.summaries.forEach(s => { if (!topicMap.has(s.topic_id)) topicMap.set(s.topic_id, s.title); });
    const topics = Array.from(topicMap.entries()).map(([id, name]) => ({ id, name }));
    const summaries = store.summaries.map(s => ({ id: s.id, topic_id: s.topic_id, content_markdown: undefined, status: s.status }));
...
export async function getStudyGoals(planId: string): Promise<StudyGoal[]> {
  if (USE_MOCKS) {
    ...
    return plan.items.map((item: StudyPlanItem) => ({
      id: item.id, plan_id: planId, topic_id: item.keyword_id || item.summary_id || '',
      topic_name: item.title, target_mastery: 0.8,
      current_mastery: item.completed ? 0.85 : 0.3,
      status: item.completed ? 'completed' as const : 'in-progress' as const,
      due_date: null, created_at: plan.created_at, updated_at: plan.created_at,
    }));
  }
...
export async function updateStudyGoal(goalId: string, data: Partial<StudyGoal>): Promise<void> {
  if (USE_MOCKS) {
    ...
    for (const plan of store.studyPlans) {
      ...
      if (itemIdx !== -1) {
        if (data.status === 'completed') plan.items[itemIdx] = { ...plan.items[itemIdx], completed: true };
        else if (data.status === 'in-progress' || data.status === 'pending') plan.items[itemIdx] = { ...plan.items[itemIdx], completed: false };
...
export async function getSmartRecommendations(_studentId: string, _limit?: number): Promise<SmartStudyRecommendation[]> {
  if (USE_MOCKS) {
    ...
    return store.smartStudy.map((item, i) => ({
      id: `rec-${item.keyword_id}`, student_id: _studentId, type: 'review-keyword' as const,
      resource_id: item.keyword_id, resource_name: item.term,
      ...
      due_date: null, status: 'pending' as const, created_at: item.last_studied || now(),
    }));
  }
...
// P0 FIX: Mock store uses field 'note' but KwStudentNote type expects 'content'.
...
export async function createKwStudentNote(kwId: string, _studentId: string, content: string): Promise<any> {
  if (USE_MOCKS) {
    await delay();
    const n: StudentNote = { id: mockId('sn'), student_id: _studentId, keyword_id: kwId, note: content, created_at: now().split('T')[0], deleted_at: null };
...
export async function getVideoNotesByVideo(videoId: string, _studentId: string): Promise<any[]> {
  if (USE_MOCKS) {
    ...
    return store.videoNotes
      ...
      .map(n => ({ ...n, content: n.note, timestamp_ms: n.timestamp_seconds != null ? n.timestamp_seconds * 1000 : null }));
  }
