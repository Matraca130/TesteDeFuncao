...
content.post("/semesters", async (c) => {
  try {
    ...
    const id = crypto.randomUUID();
    const semester = { id, course_id: body.course_id, title: body.title, year: body.year ?? null, sort_order: body.sort_order ?? 0, created_at: new Date().toISOString(), created_by: user.id };
    await kv.mset([KV.semester(id), KV.IDX.semesterOfCourse(body.course_id, id)], [semester, id]);
...
content.get("/semesters", async (c) => {
  try {
    ...
    const semesters = await getChildren(KV.PREFIX.semestersOfCourse(courseId), KV.semester);
...
content.delete("/semesters/:id", async (c) => {
  try {
    ...
    if (!sem) return notFound(c, "Semester");
    const keysToDelete: string[] = [KV.semester(id), KV.IDX.semesterOfCourse(sem.course_id, id)];
...
content.post("/sections", async (c) => {
  try {
    ...
    if (!user) return unauthorized(c);
    const body = await c.req.json();
    if (!body.title || !body.semester_id) return validationError(c, "Missing title or semester_id");
    const id = crypto.randomUUID();
    const section = { id, semester_id: body.semester_id, title: body.title, region: body.region ?? null, image_url: body.image_url ?? null, sort_order: body.sort_order ?? 0, created_at: new Date().toISOString(), created_by: user.id };
    await kv.mset([KV.section(id), KV.IDX.sectionOfSemester(body.semester_id, id)], [section, id]);
...
content.get("/sections", async (c) => {
  try {
    ...
    const sections = await getChildren(KV.PREFIX.sectionsOfSemester(semId), KV.section);
...
content.delete("/sections/:id", async (c) => {
  try {
    ...
    if (!sec) return notFound(c, "Section");
    const keysToDelete: string[] = [KV.section(id), KV.IDX.sectionOfSemester(sec.semester_id, id)];
...
content.post("/topics", async (c) => {
  try {
    ...
    if (!user) return unauthorized(c);
    const body = await c.req.json();
    if (!body.title || !body.section_id) return validationError(c, "Missing title or section_id");
    const id = crypto.randomUUID();
    const topic = { id, section_id: body.section_id, title: body.title, description: body.description ?? null, sort_order: body.sort_order ?? 0, created_at: new Date().toISOString(), created_by: user.id };
    await kv.mset([KV.topic(id), KV.IDX.topicOfSection(body.section_id, id)], [topic, id]);
...
content.get("/topics", async (c) => {
  try {
    ...
    const topics = await getChildren(KV.PREFIX.topicsOfSection(secId), KV.topic);
...
content.delete("/topics/:id", async (c) => {
  try {
    ...
    const keysToDelete: string[] = [KV.topic(id), KV.IDX.topicOfSection(topic.section_id, id)];
...
content.post("/summaries", async (c) => {
  try {
    ...
    const id = crypto.randomUUID();
    const summary = { id, topic_id: body.topic_id, institution_id: body.institution_id, title: body.title, content: body.content ?? "", status: body.status || "draft", source: body.source || "manual", created_at: new Date().toISOString(), created_by: user.id, updated_at: new Date().toISOString() };
    await kv.mset([KV.summary(id), KV.IDX.summaryOfTopic(body.topic_id, id)], [summary, id]);
...
content.get("/summaries", async (c) => {
  try {
    ...
    const summaries = await getChildren(KV.PREFIX.summariesOfTopic(topicId), KV.summary);
...
content.put("/summaries/:id", async (c) => {
  try {
    ...
    const body = await c.req.json();
    const updated = { ...existing, ...body, id, topic_id: existing.topic_id, updated_at: new Date().toISOString() };
...
content.delete("/summaries/:id", async (c) => {
  try {
    ...
    if (!summary) return notFound(c, "Summary");
    const keysToDelete: string[] = [KV.summary(id), KV.IDX.summaryOfTopic(summary.topic_id, id)];
    ...
    for (const kwId of kwIds) {
      keysToDelete.push(KV.IDX.keywordOfSummary(id, kwId), KV.IDX.summaryOfKeyword(kwId, id));
    }
    await kv.mdel(keysToDelete);
    return c.json({ success: true, data: { deleted: true } });
...
content.get("/summaries/:id/chunks", async (c) => {
  try {
    const user = await getAuthUser(c);
    if (!user) return unauthorized(c);
    const chunks = await getChildren(KV.PREFIX.chunksOfSummary(c.req.param("id")), KV.chunk);
...
content.post("/summaries/:id/chunk", async (c) => {
  try {
    ...
    const chunk = { id, summary_id: summaryId, content: body.content, sort_order: sortOrder, created_at: new Date().toISOString() };
    await kv.mset([KV.chunk(id), KV.IDX.chunkOfSummary(summaryId, sortOrder)], [chunk, id]);
...
content.post("/keywords", async (c) => {
  try {
    ...
    const id = crypto.randomUUID();
    const keyword = { id, institution_id: body.institution_id, term: body.term, definition: body.definition ?? null, priority: body.priority ?? 0, status: body.status || "draft", summary_ids: body.summary_ids || [], created_at: new Date().toISOString(), created_by: user.id };
    const keys: string[] = [KV.keyword(id), KV.IDX.keywordOfInst(body.institution_id, id)];
    const vals: any[] = [keyword, id];
    if (body.summary_ids && Array.isArray(body.summary_ids)) {
      for (const sId of body.summary_ids) {
        keys.push(KV.IDX.keywordOfSummary(sId, id)); vals.push(id);
        keys.push(KV.IDX.summaryOfKeyword(id, sId)); vals.push(sId);
      }
    }
    if (body.summary_id) {
      const instId = crypto.randomUUID();
      const instance = { id: instId, keyword_id: id, summary_id: body.summary_id, chunk_id: body.chunk_id ?? null, start_offset: body.start_offset ?? null, end_offset: body.end_offset ?? null, created_at: new Date().toISOString() };
      keys.push(KV.keywordInstance(instId)); vals.push(instance);
    }
...
content.get("/keywords", async (c) => {
  try {
    ...
    if (summaryId) {
      const keywords = await getChildren(KV.PREFIX.keywordsOfSummary(summaryId), KV.keyword);
      return c.json({ success: true, data: keywords });
    }
    if (instId) {
      const keywords = await getChildren(KV.PREFIX.keywordsOfInst(instId), KV.keyword);
      return c.json({ success: true, data: keywords });
    }
...
content.get("/keywords/:id", async (c) => {
  try {
    ...
    const subtopics = await getChildren(KV.PREFIX.subtopicsOfKeyword(id), KV.subtopic);
    ...
    return c.json({ success: true, data: { ...keyword, subtopics, connections, linked_summary_ids: summaryIds } });
...
content.delete("/keywords/:id", async (c) => {
  try {
    ...
    const summaryIds = await kv.getByPrefix(KV.PREFIX.summariesOfKeyword(id));
    const keysToDelete: string[] = [
      KV.keyword(id), KV.IDX.keywordOfInst(kw.institution_id, id),
...
content.post("/subtopics", async (c) => {
  try {
    ...
    if (!user) return unauthorized(c);
    const body = await c.req.json();
    if (!body.keyword_id || !body.title) return validationError(c, "Missing keyword_id or title");
    const id = crypto.randomUUID();
    const subtopic = { id, keyword_id: body.keyword_id, title: body.title, description: body.description ?? null, priority: body.priority ?? null, status: body.status || "draft", created_at: new Date().toISOString(), created_by: user.id };
    await kv.mset([KV.subtopic(id), KV.IDX.subtopicOfKeyword(body.keyword_id, id)], [subtopic, id]);
...
content.get("/subtopics", async (c) => {
  try {
    ...
    const subtopics = await getChildren(KV.PREFIX.subtopicsOfKeyword(kwId), KV.subtopic);
...
content.delete("/subtopics/:id", async (c) => {
  try {
    ...
    if (!st) return notFound(c, "SubTopic");
    await kv.mdel([KV.subtopic(id), KV.IDX.subtopicOfKeyword(st.keyword_id, id)]);
    return c.json({ success: true, data: { deleted: true } });
...
content.post("/connections", async (c) => {
  try {
    ...
    if (!user) return unauthorized(c);
    const body = await c.req.json();
    if (!body.keyword_a_id || !body.keyword_b_id) return validationError(c, "Missing keyword_a_id or keyword_b_id");
    const id = crypto.randomUUID();
    const conn = { id, keyword_a_id: body.keyword_a_id, keyword_b_id: body.keyword_b_id, relationship_type: body.relationship_type ?? "related", description: body.description ?? null, created_at: new Date().toISOString(), created_by: user.id };
    await kv.mset(
      [KV.connection(id), KV.IDX.connectionOfKeyword(body.keyword_a_id, id), KV.IDX.connectionOfKeyword(body.keyword_b_id, id)],
      [conn, id, id]
    );
...
content.put("/content/batch-status", async (c) => {
  try {
    ...
    const entityKeyFns: Record<string, (id: string) => string> = {
      keyword: KV.keyword, subtopic: KV.subtopic, summary: KV.summary,
    };
    const results: any[] = [];
    ...
    for (const item of items) {
      ...
      if (!keyFn) return validationError(c, `Unsupported entity_type: ${entity_type}`);
      const existing = await kv.get(keyFn(id));
      if (!existing) { results.push({ id, entity_type, success: false, reason: "not found" }); continue; }
      const updated = { ...existing, status: new_status, reviewed_at: new Date().toISOString(), reviewed_by: user.id, reviewer_note: body.reviewer_note || null };
      keysToWrite.push(keyFn(id)); valsToWrite.push(updated);
      results.push({ id, entity_type, success: true, new_status });
    }
    if (keysToWrite.length > 0) { await kv.mset(keysToWrite, valsToWrite); }
    return c.json({
      success: true,
      data: {
