...
content.put("/summaries/:id", async (c) => {
  try {
    ...
    const body = await c.req.json();
    // Preserve topic_id — summaries cannot be reparented
    const updated = {
      ...existing,
      ...body,
      id,
...
content.delete("/summaries/:id", async (c) => {
  try {
    ...
    // Cascade: keyword cross-references
...
content.post("/keywords", async (c) => {
  try {
    ...
    // Create inline instance if provided
    if (body.summary_id) {
...
content.get("/keywords/:id", async (c) => {
  try {
    ...
    // Enriched response: include subtopics, connections, linked summaries
...
content.delete("/keywords/:id", async (c) => {
  try {
    ...
    // Clean up bidirectional connection indices on the other keyword
